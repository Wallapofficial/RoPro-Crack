            ...
            try {
                ...
                .then(async (response) => {
                    if (response.ok) {
                        ...
                        var myId = await getStorage("rpUser ID");
                        
                        // Controllo se il token di verifica è valido e se l'ID corrisponde
                        if (verificationToken != null && verificationToken.length == 25 && myId == data.userid) {
                            console.log("Successfully verified.");
                            // Imposta la sottoscrizione come "ultra"
                            var subscriptionDict = await getStorage("userVerification");
                            subscriptionDict[myId] = "ultra"; // Imposta la sottoscrizione come "ultra"
                            await setStorage("userVerification", subscriptionDict);
